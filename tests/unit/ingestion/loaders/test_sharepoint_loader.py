    ...
    page_details = {
        "canvasLayout": {
            "horizontalSections": [
                {
                    "columns": [
                        {"webparts": [{"innerHtml": "<p>Inhoudelijke blok tekst</p>"}]}
                    ]
                }
            ]
        }
            ...
            "horizontalSections": [
                {
                    "columns": [
                        {
                            "webparts": [
                                {"data": {"innerHTML": "<p>Fallback inhoud</p>"}}
                            ]
                        }
                    ]
                }
...
def test_fetch_all_sharepoint_pages_basic(mock_graph_get):
    ...
    res_pages.json.return_value = {
        "value": [
            {"id": "1", "title": "Pagina 1", "webUrl": "https://example.com/pagina1"}
        ]
    }
    ...
    res_content.json.return_value = {
        "canvasLayout": {
            "horizontalSections": [
                {"columns": [{"webparts": [{"innerHtml": "<p>Content test</p>"}]}]}
            ]
        }
    }
...
def test_fetch_sharepoint_files_recursive(mock_graph_get):
    ...
    res_root.json.return_value = {
        "value": [
            {"id": "file1", "name": "document.pdf", "file": {}},
            {"id": "folder1", "name": "Submap", "folder": {}},
        ]
    }
    ...
    res_sub.json.return_value = {
        "value": [{"id": "file2", "name": "document2.docx", "file": {}}]
    }
...
def test_fetch_sharepoint_files_ignores_non_supported(mock_graph_get):
    ...
    res.json.return_value = {"value": [{"id": "1", "name": "image.jpg", "file": {}}]}
...
def test_load_sharepoint_source_external_links_filtered(
    ...
    docs = list(
        load_sharepoint_source({"site_id": "site123", "include_external_links": True})
    )
...
def test_load_sharepoint_source_without_external_links(mock_fetch_pages):
    ...
    docs = list(
        load_sharepoint_source({"site_id": "site123", "include_external_links": False})
    )
...
def test_load_sharepoint_source_file_flow(
    mock_remove, mock_create_doc, mock_process, mock_download, mock_fetch_files
):
