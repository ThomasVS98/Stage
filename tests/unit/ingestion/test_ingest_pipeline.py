import pytest
from ingestion.ingest_pipeline import load_all_data, build_index, cleanup_temp_files
...
def test_load_all_data_no_sources(monkeypatch):
    monkeypatch.setattr("ingestion.ingest_pipeline.load_source_config", lambda: [])
...
def test_load_all_data_skips_disabled_source(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: [{"type": "sharepoint", "enabled": False, "config": {}}],
    )
    ...
    monkeypatch.setattr("ingestion.ingest_pipeline.get_loader", lambda _: fake_loader)
...
def test_load_all_data_calls_loader(monkeypatch):
    monkeypatch.setattr(
        "ingestion.ingest_pipeline.load_source_config",
        lambda: [{"type": "sharepoint", "enabled": True, "config": {"a": "1"}}],
    )
...
def test_load_all_data_no_loader(monkeypatch):
    monkeypatch.setattr(
        "ingestion.ingest_pipeline.load_source_config",
        lambda: [{"type": "unknown", "enabled": True, "config": {}}],
    )

    monkeypatch.setattr("ingestion.ingest_pipeline.get_loader", lambda _: None)
...
def test_load_all_data_raises_external_service_error(monkeypatch):
    monkeypatch.setattr(
        "ingestion.ingest_pipeline.load_source_config",
        lambda: [{"type": "test", "enabled": True, "config": {}}],
    )
    ...
    monkeypatch.setattr(
        "ingestion.ingest_pipeline.get_loader", lambda _: failing_loader
    )
...
def test_build_index_inserts_documents(monkeypatch):
    ...
    monkeypatch.setattr(
        "ingestion.ingest_pipeline.create_index", lambda: (MockIndex(), mock_collection)
    )
...
def test_build_index_triggers_gc(monkeypatch):
    ...
    monkeypatch.setattr(
        "ingestion.ingest_pipeline.gc.collect", lambda: gc_calls.append(True)
    )
...
def test_cleanup_temp_files_removes_existing_dirs(monkeypatch):
    ...
    monkeypatch.setattr("ingestion.ingest_pipeline.os.path.exists", fake_exists)

    monkeypatch.setattr("ingestion.ingest_pipeline.shutil.rmtree", fake_rmtree)
...
def test_cleanup_temp_files_skips_missing_dirs(monkeypatch):
    monkeypatch.setattr("ingestion.ingest_pipeline.os.path.exists", lambda path: False)

    removed = []

    monkeypatch.setattr(
        "ingestion.ingest_pipeline.shutil.rmtree", lambda path: removed.append(path)
    )
