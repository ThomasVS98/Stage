...
def test_start_creates_session(mock_store):
    ...
    mock_store.update.assert_called_once_with(
        result["session_id"], "original_question", "Laptop werkt niet"
    )
...
def test_answer_next_step(mock_store, mock_validate):
    mock_store.get.return_value = {"step": 0, "data": {}}

    result = answer({"session_id": "abc", "answer": "test"})
...
def test_answer_not_relevant(mock_validate, mock_store, mock_relevant):
    mock_store.get.return_value = {"step": 0, "data": {"original_question": "vraag"}}

    with patch(
        "services.intake.intake_service.INTAKE_QUESTIONS", [("key", "question")]
    ):
        result = answer({"session_id": "abc", "answer": "test"})
...
@patch("services.intake.intake_service.session_store")
@patch("services.intake.intake_service.validate_answer", return_value="cleaned")
def test_answer_returns_similar_ticket(
    mock_validate, mock_store, mock_relevant, mock_match
):
    mock_store.get.return_value = {"step": 0, "data": {"original_question": "vraag"}}
...
def test_answer_creates_ticket(
    mock_validate, mock_store, mock_relevant, mock_match, mock_create
):
    mock_store.get.return_value = {"step": 0, "data": {"original_question": "vraag"}}
...
def test_answer_create_ticket_raises(
    ...
    with patch(
        ...
        with pytest.raises(ExternalServiceError):
            answer({"session_id": "abc", "answer": "test"})
